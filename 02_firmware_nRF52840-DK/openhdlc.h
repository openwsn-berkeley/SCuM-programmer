...
// without escape bytes, HDLC flags. Does contain the CRC
#define SERIAL_INPUT_BUFFER_SIZE  (1+1+1+1024+2) // worst case is a 1024-byte chunk: type, offset, lenth, chunk, crc

//=========================== typedef =========================================

typedef void (*openhdlc_rx_cbt)(uint8_t* rxFrame, uint8_t rxFrameLen);

typedef struct {
    // admin
    openhdlc_rx_cbt     openhdlc_rx_cb;
    // input
...
typedef struct {
    ...
    uint32_t            num_frames_received_err_overflow;
    uint32_t            num_frames_received_err_crc;
    // bsp
